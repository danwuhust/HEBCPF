...
\lhead{\small HEBCPF -- MEX v4.20260714}
...
{\large\bfseries Release: MEX v4.20260714}\\[4pt]
...
\subsection{Reported v3/v4 \code{parfeval} reference timings}
The following development-time reference measurements compare MEX v4 with MEX v3 on the
same 23-worker local pool. They are retained to describe the intended performance change;
they were \textbf{not re-executed during this package-preparation pass}. Each value is the
mean wall time of two runs, excluding pool start-up. The two releases were reported to
return the same solution sets under order-insensitive matching on the bundled cases through
\code{case57}.
...
Timing varies with MATLAB release, processor, pool size, and background load. Treat these
as a reference ratio rather than a performance guarantee.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_MEX_v4_20260714}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_MEX_v4_20260714'
>> addpath(pwd)
\end{lstlisting}
...
\begin{lstlisting}[style=sh]
:: SERIAL
matlab -sd "<path>\HEBCPF_MEX_v4_20260714" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_MEX_v4_20260714" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_MEX_v4_20260714" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
commented \code{save} in its ``save temporary data'' block; enable it, or simply run
\code{save mycheckpoint.mat} at any time, to write the full workspace: the model matrices
together with the running solution set \code{Zsave} and its bookkeeping matrix \code{VBook}.

To resume, \code{load} that file, (re)set \code{wait}, and run the collection script for the
same framework:
\begin{lstlisting}[style=mat]
...
HEBCPF --- MEX v4.20260714. This document accompanies the solver release and may be
