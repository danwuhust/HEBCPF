...
{\large\bfseries Release: MATLAB v2.202607}\\[4pt]
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      ...
      x64, use the companion compiled build (\code{HEBCPF_MEX_v2.202607}); the numerical
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v2.202607}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_matlab_v2.202607'
>> addpath(pwd)
\end{lstlisting}
...
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v2.202607" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v2.202607" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v2.202607" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v2.202607}
