...
%%
%% Wrap large read-only matrices as parallel.pool.Constant
%  transferred to each worker once after the parpool starts, avoiding rebroadcast on every while iteration
...
while numberofsolutions+1>solutionnumber
    ...
    parfor eqnN = 1:Neqnqueue
        ...
        [Z_result, Time_result, NY_inc_result] = trace_equation_worker( ...
            MS_c.Value, M0_c.Value, Tinv_c.Value, Ma_c.Value, ...
            Mp_c.Value, Mq_c.Value, Ybus_c.Value, ...
