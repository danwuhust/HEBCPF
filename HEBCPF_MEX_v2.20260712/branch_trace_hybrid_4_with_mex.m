...
% set  next step size
        ...
        astep = max(astep,param.minimumstep/10);
...
%                   fprintf('=================================================================\n\n')
           ...
           x = x+ dx;
           alpha = alpha + dy(Nx+1);
           if (norm(F)<5*param.ef)&&(norm(dx,inf)<10*param.ex)
              break;
           end
        end
    end  
...
%             fprintf('Numerically unstable! Back up.\n')
        ...
        sarc(actstep) = snew;
        dsm1 = ds;
         astep = astep*(1 + (param.targetsteps-icount)/40);
         astep = min(astep,param.maximumstep);  % maximum step
         astep = max(astep,param.minimumstep); % minstep
...
%          sgn=sign(Y(end,actstep)-Y(end,actstep-10));       
     ...
     if cross_singular==1
         ...
         if secant_slope<=param.slope_max && abs(Y(end,actstep)-paal)>=param.handback*param.aal_min % default 1.2*param.aal_min
             break;
         end
     end
     ...
     if escape==1 % escape from zero constant point
         ...
         if min(abs(dba))>=1.*10^(-6) % 1.5*10^(-4)
             secant_slope=norm((Y(1:end-1,actstep)-Y(1:end-1,actstep-1))/(Y(end,actstep)-Y(end,actstep-1)),inf);
