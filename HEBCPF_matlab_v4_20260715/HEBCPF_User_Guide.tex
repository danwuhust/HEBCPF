...
\lhead{\small HEBCPF -- MATLAB v4.20260715}
...
{\large\bfseries Solver line: MATLAB v4.20260715}\\[2pt]
{\large\bfseries Package release: 2026.07.15}\\[4pt]
{\large Pure-MATLAB build (no compiled binaries; cross-platform) --- portable v4 reference}\\[12pt]
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      with no MEX / codegen step. On Windows x64, the companion compiled build
      (\code{HEBCPF_MEX_v4_20260715}) completed the release queue-\code{parfeval}
      benchmark in $1/1.77$ of this package's aggregate wall time; see
      Section~\ref{sec:benchmark} for scope and limits.
\end{description}
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v4_20260715}) anywhere on disk.
...
\end{lstlisting}
  \item Add only one HEBCPF solver folder to the MATLAB path at a time. Do not mix this
        folder with the MEX or older-release folders because they contain many functions
        and cases with the same names.
...
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v4_20260715" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v4_20260715" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v4_20260715" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
\code{slope_max}    & \code{4e5}  & Maximum secant slope at which a turning point is accepted as passed in this v4 release. Validate difficult large cases before changing this threshold.\\
...
workers. Parallel-pool startup was excluded from the per-case timings. Pure MATLAB v4
completed the suite in 1272.966~s (1271.222~s trace time), while the MEX v4 companion
required 718.656~s (716.062~s trace time): the MEX package used $1.77\times$ less aggregate
wall time in this environment. Pure MATLAB was faster on six small cases, where fixed
overhead dominates.
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v4_20260715}
ships compiled Pad\'e and holomorphic kernels plus its KLU-enabled corrector path. In the
released 20-case queue-\code{parfeval} benchmark it used $1.77\times$ less aggregate wall
time, while both packages matched solution counts and solution sets within the shared
\code{4e-7} tolerance. This is an environment-specific package comparison, not a promise of
the same speed ratio for serial runs or other platforms. The two builds are otherwise
...
HEBCPF --- MATLAB v4.20260715. This document accompanies the solver release and may be
