...
\lhead{\small HEBCPF -- MATLAB v5.2}
...
{\large\bfseries Solver line: MATLAB v5.2}\\[2pt]
{\large\bfseries Package version: v5.2.0}\\[4pt]
{\large Pure-MATLAB build (no compiled binaries; cross-platform) --- portable v5.2 reference}\\[12pt]
...
{\small Documentation updated: July 30, 2026}
\end{titlepage}
...
\subsection{What v5 adds to v4}\label{sec:v4}
The pure-MATLAB solver preserves the complete v4 formulation, numerical controls,
installation workflow, parameter reference, and portability guarantees while matching
the MEX v5 queue semantics. It adds \code{scan}, \code{bandit} (default), and
\code{novelty}; per-equation cursors; anytime metrics; policy-portable checkpoints;
and centralized 2,000 initial/5,000 hard encounter caps. Every hot-path call remains
MATLAB code. Scheduler choice changes work order, not equations or acceptance tolerance.
...
\item[MATPOWER (required).] MATPOWER must be installed and on the MATLAB path. The setup
      ...
      \code{ext2int} for internal numbering, and \code{idx_bus}/\code{idx_gen}/
      \code{idx_brch} for MATPOWER matrix indices. The solver reads the
      bundled MATPOWER-format \code{caseXXX.m} data files, but does \emph{not} ship
      \code{runpf}, \code{makeYbus}, \code{ext2int}, \code{idx_bus}, \code{idx_gen},
      \code{idx_brch}, or their
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      with no MEX / codegen step. For compiled hot paths on Windows x64, use the
      companion \code{HEBCPF_MEX_v5.2} build; both packages use the same HEBC formulation,
      cases, tolerances, and scheduler semantics.
\item[Linear-solver boundary.] This implementation uses MATLAB's built-in linear-algebra
      routines and never calls \code{klurf}. Copying a KLU binary into this folder does not
      enable KLU acceleration. Use the companion \code{HEBCPF_MEX_v5.2} solver when the
      shipped KLU-accelerated corrector is required.
\end{description}

\subsection{V5.2 sparse preprocessing}

All serial and parallel entry points first normalize MATPOWER cases with
\code{ext2int}. The function \code{get_quadr_mtrx.m} then constructs sparse active- and
reactive-power quadratic forms directly from the real and imaginary parts of
\code{Ybus}; it does not allocate the dense three-dimensional matrices used by the V5.0
implementation. This formulation also handles a non-symmetric \code{Ybus}, including
phase-shifting transformer models.

\code{quadr_matrix.m} preserves the established slack-voltage, PV-voltage, PQ-reactive,
PV-active, and PQ-active equation order. Scheduled active power is summed over all online
generators at each PV bus. Online generators sharing a PV or reference bus must have a
consistent voltage setpoint. The preprocessing reports an explicit error for missing
online generators, inconsistent setpoints, unsupported bus types, or a reference-bus
count other than one.

\subsection{V5.2 pure-MATLAB versus MEX comparison}\label{sec:matmex}

The two V5.2 packages were compared on all 20 bundled cases through \code{case57} using
the asynchronous \code{parfeval} queue, the \code{bandit} scheduler, three repeats per
case, and one shared persistent 23-worker pool. Pool startup was excluded. The host was an
Intel Core i9-13900K (24 cores/32 threads) with 128~GB RAM, Windows 11 Pro x64, and MATLAB
R2022a (9.12) on \code{PCWIN64}; the run was dated July 27, 2026.

\IfFileExists{matlab_benchmark.tex}{\input{matlab_benchmark.tex}}{%
\textit{The V5.2 comparison table is unavailable in this copy of the documentation.}}

The comparison supports case-by-case agreement in completed solution counts. The
preserved artifact does not include raw repeat records or per-solution residual vectors,
so this release makes no new residual-distance claim from these timing data. The full
GitHub-readable table and provenance boundary are in the top-level
\code{MATLAB_BENCHMARK_V5.2.md} and \code{MATLAB_BENCHMARK_METADATA_V5.2.json} files.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v5.2}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_matlab_v5.2'
>> addpath(pwd)
\end{lstlisting}
  \item Add only this one HEBCPF release folder to the MATLAB path. Do not add both
        solver folders simultaneously, because they contain many functions and cases with
        the same names.
...
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v5.2" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v5.2" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v5.2" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
All three frameworks target the same completed solution set; they differ in how the
per-equation traces are distributed. Equivalence is verified by final count, residual,
and order-independent set distance rather than inferred from timing.
\begin{description}[leftmargin=0em,style=nextline]
...
\code{scheduler} & normalized queue policy used by \code{parfeval}\\
\code{trace_metrics} & total traces, t50/t90/t99, wall-to-90\%, selection cost, and histories\\
...
saved workspace instead of restarting. The \code{parfor} and queue \code{parfeval} runbooks
write periodic checkpoints to \code{temp_result.mat}. The saved state contains the model
matrices, the current solution matrix \code{Zsave}, and its bookkeeping matrix \code{VBook};
runtime-only futures and pools are rebuilt on resume.

To resume a ceased \code{parfeval} or \code{parfor} search, return to the same solver
folder, use the same case and load setting, load \code{temp_result.mat}, and run the same
collection script:
\begin{lstlisting}[style=mat]
...
\end{lstlisting}
The serial driver does not maintain a periodic checkpoint by default. Before stopping a
serial run, save the workspace manually with \code{save('mycheckpoint.mat','-v7.3')}; resume
with \code{load mycheckpoint.mat}, \code{wait = 0}, and
\code{run('runVBook_hybrid_2023.m')}.

On start-up the script checks for an existing \code{VBook}. If \code{Zsave} is longer than
\code{VBook}, it trims \code{Zsave} to the book-kept solution count; if their sizes match,
it sets \code{initbypass} and continues without re-seeding. The queue \code{parfeval} driver
rebuilds its per-equation cursors from the zero entries of \code{VBook}; all drivers re-seed
the keyed deduplicator (Section~\ref{sec:dedup}) from the loaded solutions, so a large
checkpoint continues at full speed.

\input{../HEBCPF_Scheduler_Guide.tex}
...
This v5 build ships a default configuration that hardens the tracer
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Turning-point slope limit} \code{slope_max = 4e5} --- keeps the
        continuation active through steep near-singular arcs instead of handing back too
        early; validate difficult case118 curves before treating this threshold as a
        universal default;
...
\subsection{Linear solves in the pure-MATLAB implementation}\label{sec:linear}
The continuation corrector and \code{Resolve_no_mex} use MATLAB's built-in linear-algebra
routines. This solver neither checks \code{USEKLU} nor calls \code{klu} or \code{klurf};
placing either MEX binary on the MATLAB path therefore does not change its execution path.
For the KLU symbolic-refactor implementation and its build requirements, use the companion
\code{HEBCPF_MEX_v5.2} solver and its user guide.
...
solution set, which is $O(N)$ per candidate and $O(N^2)$ over a run. This linear scan can
therefore dominate as the saved solution set grows, especially when resuming a long run.
...
the serial, parfor, and parfeval collectors alike. Solution counts were unchanged in the
released \code{case9}/\code{case14mod} validation. The bucket lookup avoids scanning every
saved solution for each candidate, which is especially useful when resuming a long run.
...
\begin{center}\small
\begin{longtable}{@{}p{0.24\textwidth}p{0.16\textwidth}p{0.53\textwidth}@{}}
\toprule
...
\code{maxsolu}      & \code{2000} & Initial allocated encounter capacity per traced loop.\\
\code{maxsolu_hard} & \code{5000} & Hard encounter cap; stops cleanly before storage overrun.\\
...
\code{slope_max}    & \code{4e5}  & Maximum secant slope at which a turning point is accepted as passed in this v5 release. Validate difficult large cases before changing this threshold.\\
...
\input{../HEBCPF_Scheduler_Benchmark_Guide.tex}
...
\code{ext2int} and \code{makeYbus} from MATPOWER (internal numbering and Ybus),
\code{get_quadr_mtrx.m} (sparse power-balance quadratics),
\code{quadr_matrix.m} (online-generator aggregation and sparse PV/PQ/slack matrices),
\code{Solu.m} (initial point from
...
\textbf{V5.2 comparison:} \code{matlab_benchmark.tex/png} (embedded pure-vs-MEX table
and figure); top-level \code{MATLAB_BENCHMARK_V5.2.md/json} files provide the readable
report and provenance metadata.
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v5.2}
ships compiled Pad\'e and holomorphic kernels plus the KLU corrector. In the published
23-worker \code{parfeval} comparison, the rounded table gives a 1.95$\times$ aggregate
wall-time ratio and a 1.59$\times$ geometric-mean case-wise wall-time factor in favor of
MEX, with matching completed solution counts on all 20 cases. These are parallel
same-machine measurements, not a claim that every workload will see the same speedup.
...
The companion MEX solver uses \textbf{SuiteSparse/KLU} (Timothy A.~Davis), distributed
under its own license terms. This pure-MATLAB solver does not link to or call KLU. The
top-level \code{NOTICE}, \code{klurf/README_KLURF.md}, and V5 corresponding-source release
asset describe the MEX component and its third-party requirements.
...
\begingroup
\small
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,parsep=0pt]
...
\item D.\ Wu and B.\ Wang, ``Collection of Numerous Power Flow Solutions of Standard IEEE
      ...
      doi:\,\doi{10.21227/24bh-hj72}. \emph{(Reference solution sets for validating
      solver output.)}
\end{enumerate}
\endgroup
