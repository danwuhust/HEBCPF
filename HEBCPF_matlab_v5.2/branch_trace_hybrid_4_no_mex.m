...
% Pure-MATLAB v4: corrector solves use MATLAB's built-in backslash only.
...
%             Z = [Z z];
            ...
            if scount>maxsolu_cur
                % Cap hit: leave THIS curve unclosed (Fail=2, break); members
                % re-reached from other starts (completeness holds). Ratchet
                % the cap up for future curves (per worker; <= maxsolu_hard).
