...
%% ---- solver options ----------------------------------------------------
% Trace-scheduling policy for runVBook_hybrid_parfeval: scan, bandit (default),
% or novelty. Legacy v5 'diverse' maps to bandit. See POLICY_README.md.
% Change the value here, then run main.m followed by runVBook_hybrid_parfeval.
% (No effect on the serial / parfor drivers.)
...
% mpc=case4gs; % 4 solutions % 2.55 s
...
mpc = ext2int(mpc); % use consecutive internal MATPOWER bus numbering
...
%
if result.success == 1
result = ext2int(result); % align the solved voltage vector with internal indexing
%% obtain quadratic matrices of power balancing equations
tic;
fprintf('\nobtain quadratic matrices of power balancing equations...\n');
[Mp,Mq]=get_quadr_mtrx(Ybus,bus_n);% compute sparse quadratic matrices for power balance equations
toc;
%
...
[Ma,ba,I]=quadr_matrix(Mp,Mq,mpc,bus_n,gen_n,numofvar);% compute all the quadratic matrices
