...
benchmark_policy = strtrim(getenv('HEBCPF_BENCH_POLICY'));
...
metadata.scheduler_policy = benchmark_policy;
...
for si = 1:numel(solvers)
    ...
    for ci = 1:numel(cases_to_run)
        ...
        try
            ...
            [HEBCPOLICY,~] = trace_policy_config(benchmark_policy);
            record.scheduler_policy = HEBCPOLICY;
            ...
            record.n_traces_recorded = local_get_if_exists('count', double(max(VBook(:))));
            if exist('trace_solution_history','var') && ~isempty(trace_solution_history)
                target90 = ceil(0.9*numberofsolutions);
                record.trace_to_90pct = find(double(trace_solution_history) >= target90,1,'first');
            end
