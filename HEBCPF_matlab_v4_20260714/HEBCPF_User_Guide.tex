...
\lhead{\small HEBCPF -- MATLAB v4.20260714}
...
{\large\bfseries Release: MATLAB v4.20260714}\\[4pt]
{\large Pure-MATLAB build (no compiled binaries; cross-platform) --- performance-optimized, robustness-hardened}\\[12pt]
...
\subsection{What is new in pure MATLAB v4}\label{sec:v4}
This release is the portable counterpart of MEX v4. It retains the v4 numerical
configuration and all user-facing workflows while keeping every execution path in
MATLAB \code{.m} code:
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Global \code{parfeval} work queue.} One trace per equation may be in
        flight at a time, avoiding v3 row barriers while preserving VBook pruning.
  \item \textbf{Cached built-in sparse LU ordering.} \code{lu_cached.m} reuses the
        column ordering for the holomorphic matrix within a case; it calls MATLAB's
        built-in \code{lu}, not a compiled solver.
  \item \textbf{Sparse holomorphic assembly reuse.} Network invariants and stacked
        quadratic-form operators are cached, and diagonal blocks are constructed directly
        as sparse matrices.
  \item \textbf{Allocation-free Pad\'e evaluation.} Voltage polynomials are evaluated
        by matrix-vector products rather than replicated temporary power matrices.
  \item \textbf{Pure-MATLAB correctors.} \code{branch_trace_hybrid_4_no_mex.m} and
        \code{Resolve_no_mex.m} use MATLAB backslash exclusively; this release neither
        ships nor calls \code{klurf}.
\end{itemize}
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      ...
      x64, use the companion compiled build (\code{HEBCPF_MEX_v4_20260714}); the numerical
      results are identical.
\item[KLU is not used in this release.] The pure MATLAB v4 correctors deliberately use
      MATLAB backslash only, even when a \code{klurf} binary is present on the path. Use
      \code{HEBCPF_MEX_v4_20260714} for the KLU-enabled compiled corrector.
\end{description}

\paragraph{KLU note.}
SuiteSparse/KLU is relevant only to the compiled MEX releases. Its source and license
information remain at the top level for users of the MEX v4 release; no compiler or KLU
installation is needed for pure MATLAB v4.
% ================= 3. INSTALLATION =================
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v4_20260714}) anywhere on disk.
...
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v4_20260714" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v4_20260714" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v4_20260714" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
This v4 build ships a \emph{validated} default configuration that hardens the tracer
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Turning-point slope limit} \code{slope_max = 4e5} --- the shared v4
        release default; validate partial case118 enumerations separately before changing it;
...
\subsection{Linear solver implementation}\label{sec:linear}
Pure MATLAB v4 uses MATLAB built-ins throughout. The continuation and resolving correctors
form their bordered Newton systems and solve them with MATLAB backslash. The holomorphic
step uses \code{lu_cached.m}: the first sparse \code{lu} factorisation records a column
ordering, and subsequent factorisations reuse that ordering through MATLAB's own \code{lu}
interface. No KLU, \code{klurf}, or compiled code path is selected by this release.
...
\code{slope_max}    & \code{4e5}  & Maximum secant slope at which a turning point is accepted as passed. Shared v4 default; validate partial case118 enumerations separately before changing it.\\
...
\code{lu_cached.m} (cached built-in sparse-LU ordering),
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v4_20260714}
...
This pure MATLAB release does not link or call SuiteSparse/KLU. KLU-related source and
license information at the top level applies to the optional compiled MEX releases.
...
HEBCPF --- MATLAB v4.20260714. This document accompanies the solver release and may be
