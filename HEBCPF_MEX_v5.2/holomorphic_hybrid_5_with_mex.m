...
% [v4] cached-column-order LU: H's pattern is constant per case, so the
% UMFPACK column analysis runs once and 3-output GPLU is used thereafter.
% Produces the same H(Ppm,cv)=Lf*Uf identity as the stock 4-output lu.
[Lf,Uf,Ppm,Qqm]=lu_cached(H);
