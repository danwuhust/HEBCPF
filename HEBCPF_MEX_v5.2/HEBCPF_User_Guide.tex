...
{\large\bfseries Release line: MEX v5.2}\\[4pt]
{\large Windows 64-bit (compiled MEX) --- performance-optimized, robustness-hardened build}\\[30pt]
...
V5 retains the v4 HEBC formulation, case format, MEX numerical kernels,
consensus Pad\'e estimator, continuation controls, and keyed deduplication.
Its changes concentrate on asynchronous work selection and observability:
\begin{itemize}[leftmargin=1.4em]
  \item rotating \code{scan}, gain-directed \code{bandit} (default), and
        projection-based \code{novelty} queue policies;
  \item per-equation cursors replacing duplicated v4 pending lists;
  \item per-trace solution, equation, completion-time, and worker-time histories;
  \item policy-aware checkpoints that can resume under another scheduler;
  \item centralized 2,000 initial and 5,000 hard per-loop encounter caps; and
  \item an official three-repeat, 20-case V5 benchmark with performance and
        connectivity plots; benchmark metadata records both KLU binary hashes.
\end{itemize}
Scheduling changes the next eligible trace. It does not change the equations,
branch tracer, deduplication tolerance, or completed-search stopping rule.
...
\item[Compiled MEX (shipped).] Two hot-path functions are provided as Windows 64-bit
      ...
      accelerate the numerical hot path. In the V5.2 comparison they preserve the same
      completed solution count on every tested case. On Linux/macOS, or after editing hot-path source, you must
...
count other than one. This setup is shared with the pure-MATLAB v5.2 package; the MEX
distinction begins in the tracing and corrector hot paths.
...
\begin{enumerate}[leftmargin=1.6em]
  \item Get the pinned SuiteSparse v7.12.2 source. The V5 source release asset
        already contains the exact required tree at commit
        \code{42151688813c45846a597edcb601435a0e38f3dd}; alternatively clone it:
\begin{lstlisting}[style=sh]
git clone --depth 1 --branch v7.12.2 https://github.com/DrTimothyAldenDavis/SuiteSparse
\end{lstlisting}
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_MEX_v5.2}) anywhere on disk.
...
\begin{lstlisting}[style=sh]
:: SERIAL
matlab -sd "<path>\HEBCPF_MEX_v5.2" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_MEX_v5.2" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_MEX_v5.2" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
only tunable kept outside this file is the Pad\'e \code{degree}, which is baked into the
MEX binaries --- see Section~\ref{sec:params}.)
...
saved workspace instead of restarting. The parallel runbooks write periodic checkpoints to
\code{temp_result.mat}. The saved state contains the model matrices, the current solution
matrix \code{Zsave}, and its bookkeeping matrix \code{VBook}; runtime-only futures, pools,
and optional \code{parallel.pool.Constant} objects are rebuilt on resume.
...
\end{itemize}
Unless you are experimenting, leave these defaults as shipped; the tables in
Section~\ref{sec:params} describe each setting.
...
\code{slope_max}    & \code{4e5}  & Maximum secant slope at which a turning point is accepted as passed. Validate difficult case118 curves before generalizing this threshold.\\
...
\code{hybrid_traceloops_4_with_mex.m} (outer trace-loop orchestrator; loads \code{param}
from \code{solver_params.m}, primes the cached operators), \code{holomorphic_hybrid_5_with_mex.m} (one
...
The KLU acceleration (Section~\ref{sec:klu}) links \textbf{SuiteSparse/KLU}
(Timothy A.~Davis), distributed under the GNU LGPL-2.1-or-later license. The \code{klurf}
MEX source and build helper are included. The V5 release assets also contain the
exact SuiteSparse v7.12.2 component sources, complete applicable license texts,
build metadata, binary, and checksums; see \code{klurf/README_KLURF.md}.
