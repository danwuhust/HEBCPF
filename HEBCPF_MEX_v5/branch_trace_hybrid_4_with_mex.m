function [Z,Y,Fail,sgn,cmplt]=branch_trace_hybrid_4_with_mex(x,x0,M0,MS,km0,bbt,bb,...
    svec,param,sgn0,aal,Dspl,Fail,Y_start,purpose,Tinv,escape,solutionnumber,equationnumber)

% clear F J Y Z

persistent klurf_ok                                   % [KLU] cache: is the klurf MEX present?
if isempty(klurf_ok); klurf_ok = (exist('klurf','file')==3); end
persistent maxsolu_cur                                % ratcheting per-curve solution-encounter cap
if isempty(maxsolu_cur); maxsolu_cur = param.maxsolu; end
maxsolu_cur = max(maxsolu_cur, param.maxsolu);        % honor config increases; never lower, only ratchet up
global USEKLU; %#ok<GVMIS>                             % optional corrector-mode override
...
%}   
    ...
    % corrector
    % [KLU] Phase-I bordered solve via KLU "symbolic-once" refactor (klurf): factor the
    % sparse power-flow core once (klu_analyze), reuse the ordering and refactor each
    % Newton iteration, eliminating the 2x2 dense border by Schur complement. Exact
    % Jacobian (quadratic convergence, exact solution counts preserved). Default ON when
    % the klurf MEX is present, dense fallback otherwise. klumode: 0 dense, 1 klu
    % full-factor, 2 klurf refactor. Set global USEKLU to override the auto default.
    if isempty(USEKLU); klumode = 2*klurf_ok; else; klumode = USEKLU; end
    useklu = klumode > 0;
    for icount = 1:param.imax
        b=2*M0*x;
        d0=x'*b/2;
        if useklu && param.Dind == 0
           % KLU + Schur bordered solve; sparse core straight from MakeJacobianD('sparse')
           % (no dense J1, no per-iteration sparse() cast).
           Jc  = MakeJacobianD(x,MS,Nm,'sparse');        % Nm x Nm sparse power-flow core = A
           Fc  = Jc*x/2 + d0*km0 - bbt - svec*alpha;     % power residual (Nm x 1)
           ds  = [(x-xm1); (alpha-am1)];
           arc = (ds'*ds) - (astep*param.sstep)^2;       % arclength residual
           F   = [Fc; arc];                              % (Nm+1) residual for the conv test
           Bk  = [-svec, param.fac*km0];                 % Nm x 2 border columns
           Ck  = [2*ds(1:Nm)'; b'/param.fac^2];          % 2 x Nm border rows
           Dk  = [2*ds(Nm+1), 0; 0, -1/param.fac];       % 2 x 2 corner
           if klumode == 2
               GH = klurf(Jc, [Fc Bk]);                  % analyze-once then refactor
           else
               LUk = klu(Jc); GH = klu(LUk,'\',[Fc Bk]); % klu full factor each iteration
           end
           gk = GH(:,1); Hk = GH(:,2:end);
           y2 = (Dk - Ck*Hk) \ ([arc;0] - Ck*gk);
           dy = -[gk - Hk*y2; y2];
           dx = dy(1:Nx); x = x + dx; alpha = alpha + dy(Nx+1);
           if (norm(F)<param.ef)&&(norm(dx,inf)<param.ex)
              break;
           end
           continue;
...
%             Z = [Z z];
            ...
            if scount>maxsolu_cur
                % Cap hit: leave THIS curve unclosed (Fail=2, break). Its
                % encountered solutions are recorded; un-visited members are
                % re-reached from other starts, so completeness holds. A curve
                % this rich signals more budget is needed, so ratchet the cap
                % up for future curves (per worker; capped by maxsolu_hard).
                Fail(solutionnumber,equationnumber)=2;
                if maxsolu_cur < param.maxsolu_hard
                    old_cap = maxsolu_cur;
                    maxsolu_cur = min(2*maxsolu_cur, param.maxsolu_hard);
                    fprintf('[maxsolu] trace (%d,%d): %d encounters > cap %d -- curve left unclosed; future cap raised to %d\n', ...
                        solutionnumber, equationnumber, scount, old_cap, maxsolu_cur);
                else
                    fprintf('[maxsolu] trace (%d,%d): %d encounters at hard ceiling %d -- curve left unclosed\n', ...
                        solutionnumber, equationnumber, scount, param.maxsolu_hard);
                end
                break;
