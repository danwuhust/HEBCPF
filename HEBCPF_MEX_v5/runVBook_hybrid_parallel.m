...
sigvars_dd = sort(randperm(rs_dd, V_dd, min(7,V_dd)));   % up to 7 predefined random signature vars (>=numofvar =>full-vector match)
dd = KeyedDedup(sigvars_dd, 4*10^(-7), false, max(1024, numberofsolutions), [], 'L1');
dd.seed(Zsave(:, 1:numberofsolutions));

%% Tunable constants -- keep this driver aligned with the worker tracer.
param = solver_params();
param.fac=fac;
