...
param.maxsolu       = 2000;     % INITIAL cap on solution encounters (level crossings) per
                                %   continuation loop. RATCHETING: a curve exceeding it is left
                                %   UNCLOSED (Fail=2) and the cap doubles for future curves (per
                                %   worker process). Completeness preserved -- un-visited members
                                %   of an abandoned curve are re-reached from other starts.
param.maxsolu_hard  = 5000;     % HARD ceiling on the ratchet. MEMORY BOUND: each in-flight
                                %   trace holds a (2*bus_n-1) x maxsolu_cur buffer Z on its
                                %   worker (~9.4 MB/worker at 5000 on case118's 235 vars). Raise
                                %   only with RAM headroom: Z_MB ~= maxsolu_hard*(2*bus_n-1)*8/1e6.
param.slope_max     = 4e5;      % max secant slope to accept a turning point as passed
                                %   v4 release default; validate hard cases before changing globally
