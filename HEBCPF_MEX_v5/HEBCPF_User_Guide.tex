...
{\large\bfseries Release: MEX v5}\\[4pt]
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_MEX_v5}) anywhere on disk.
...
\begin{lstlisting}[style=sh]
:: SERIAL
matlab -sd "<path>\HEBCPF_MEX_v5" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_MEX_v5" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_MEX_v5" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
