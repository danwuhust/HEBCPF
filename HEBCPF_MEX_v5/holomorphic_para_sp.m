...
% [PROTO C] vectorized P0/Q0 + cached construction invariants.
%
% Changes vs stock:
%  1. The per-bus loop  P0(i)=solu0'*Mp{i}*solu0  (2*bus_n cell-array
%     Hermitian forms per holo step) is collapsed into one sparse mat-vec
%     against a persistent stacked operator -- the same trick as
%     quad_form_vals / MakeJacobianD.
%  2. The six spdiags() calls (M-code wrapper + makeSparseGeneral, measured
%     ~2.6 s on case39) become direct sparse(idx,idx,v) constructions.
%  3. G=real(Ybus), B=imag(Ybus), the zero block, and the deleted-column
%     list dl are constant per case and cached.
% Cache key: bus_n + nnz(Ybus) + sum|Ybus| (O(nnz), collision-proof in
% practice; refreshed automatically when a different case is loaded).
...
if isempty(nc) || key_n~=nc || key_nnz~=nnzYc || key_sum~=sumYc
    ...
    Gc = real(Ybus); Bc = imag(Ybus);
    spzc = sparse(bus_n,bus_n);
    dlc = [bus_n+I.slack 2*bus_n+I.slack 3*bus_n+I.slack 4*bus_n+I.pq' 4*bus_n+I.slack];
    idxc = (1:bus_n)';
    nc = key_n; nnzYc = key_nnz; sumYc = key_sum;
end

V0 = solu0(1:bus_n) + 1i*solu0(bus_n+1:end);
W0 = 1./V0;
N = 2*bus_n;
P0 = reshape(VP*solu0, N, bus_n).' * solu0;   % P0(i) = solu0'*Mp{i}*solu0
