...
param.slope_max=4*10^5;         % default 1*10^4
...
%  Headless/-batch: wrap matrices as Constant (sent to each worker once).
%  Desktop/GUI: pass raw matrices (avoids the R2022a pool.Constant crash).
%  getv() unwraps a Constant (.Value) or returns a raw matrix unchanged, so the
%  parfor body below is identical in both modes.
%  DEFAULT: pass matrices directly (no pool.Constant) -- stable on R2022a desktop
%  AND -batch, and as fast as pool.Constant for these sizes (case57mod 273s vs
%  287-296s + shutdown heap-corruption with Constant). Opt in to pool.Constant for
%  very large systems via SOLVER_USE_POOL_CONST=true.
...
while numberofsolutions+1>solutionnumber
    ...
    for eqind0 = 1:Neqnqueue
        ...
        for eqind1 = 2:size(New_solu{eqind0},2)
            dZoldnew1 = sum(abs(Zsave(variablelist,:)-New_solu{eqind0}(variablelist,eqind1)),1);
            if Zsave_temp_count > 0
                dZoldnew2 = sum(abs(Zsave_temp(variablelist,1:Zsave_temp_count)-New_solu{eqind0}(variablelist,eqind1)),1);
            ...
            ddZoldnew1 = sum(abs(Zsave(variablelist,:)+New_solu{eqind0}(variablelist,eqind1)),1);
            if Zsave_temp_count > 0
                ddZoldnew2 = sum(abs(Zsave_temp(variablelist,1:Zsave_temp_count)+New_solu{eqind0}(variablelist,eqind1)),1);
