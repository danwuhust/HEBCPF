...
param.imax = 40;
param.smax = 20000;
param.maximumstep = 8000;
param.maxphase1 = param.maximumstep/2;
param.maxphase2 = param.maxphase1/10;
param.minimumstep = 1e-2;
param.dev=20;
param.skipindex=0;
param.ef=1e-8;
param.ex=1e-7;
...
%% special settings
param.mismatch_error=0.1*10^-3;
param.holonum=50;
param.holo_arc_max=0.01;
param.outnum=500;
param.slope_max=4*10^5; % It affects the numerical stability in different systems and curves. Smaller value such as 1e4 is more reliable but slower.
param.interval=1;
param.aal_min=0.1*10^(-5);
param.im_tol=1*10^(-3);
...
while numberofsolutions+1>solutionnumber
    ...
    while nCompleted < Neqnqueue
        ...
        for eqind1 = 2:size(Z_result, 2)
            z = Z_result(:,eqind1);   % variablelist is 1:numofvar = all rows of Zsave
            % Positive match: test the confirmed solutions (Zsave) first, then this
            % round's newly found ones (Zsave_temp). Short-circuit on first hit.
            % (Indices match the original find over [Zsave Zsave_temp]: temp is
            %  offset by size(Zsave,2).)
            eqind2 = find(sum(abs(Zsave - z),1) < 4*10^(-7), 1);
            if isempty(eqind2) && Zsave_temp_count > 0
                j = find(sum(abs(Zsave_temp(:,1:Zsave_temp_count) - z),1) < 4*10^(-7), 1);
                if ~isempty(j); eqind2 = size(Zsave,2) + j; end
            end
            % Negative match only when there is no positive match (same precedence
            % as before) -- the common positive hit now skips three full passes.
            eqind3 = [];
            if isempty(eqind2)
                eqind3 = find(sum(abs(Zsave + z),1) < 4*10^(-7), 1);
                if isempty(eqind3) && Zsave_temp_count > 0
                    j = find(sum(abs(Zsave_temp(:,1:Zsave_temp_count) + z),1) < 4*10^(-7), 1);
                    if ~isempty(j); eqind3 = size(Zsave,2) + j; end
                end
            end
