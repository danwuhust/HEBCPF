...
{\large\bfseries Release: MEX v3.202606}\\[4pt]
{\large Windows 64-bit (compiled MEX) --- validated, robustness-hardened build}\\[30pt]
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_MEX_v3.202606}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_MEX_v3.202606'
>> addpath(pwd)
\end{lstlisting}
...
:: SERIAL
matlab -sd "<path>\HEBCPF_MEX_v3.202606" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_MEX_v3.202606" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_MEX_v3.202606" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
