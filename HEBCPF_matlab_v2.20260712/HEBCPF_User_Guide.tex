...
\lhead{\small HEBCPF -- MATLAB v2.202607}
...
{\large\bfseries Solver line: MATLAB v2.202607}\\[2pt]
{\large\bfseries Package release: 2026.07.12}\\[4pt]
{\large Pure-MATLAB build (no compiled binaries; cross-platform)}\\[30pt]
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      ...
      x64, use the companion compiled build (\code{HEBCPF_MEX_v2.20260712}); the numerical
      results are identical.
\item[KLU acceleration (optional).] The bordered Newton solve can be accelerated by the
      \code{klurf} MEX (built from SuiteSparse/KLU). This pure-MATLAB build does not ship it
      and uses the dense solve by default; drop a \code{klurf} binary into the folder --- or
      build one (top-level \code{klurf/README_KLURF.md}) --- to enable it
...
The \code{klurf} MEX links KLU from \textbf{SuiteSparse} (SPDX \code{LGPL-2.1+}). This
pure-MATLAB release does not bundle it; building it is the only step that needs a compiler,
and it is entirely optional (the dense solve runs everywhere without it). You do \emph{not}
need a system-wide SuiteSparse install or its CMake build --- \code{klurf_make.m} compiles
just the handful of KLU / AMD / COLAMD / BTF sources it needs, so a working MATLAB
\code{mex} compiler is the only prerequisite.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v2.20260712}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_matlab_v2.20260712'
>> addpath(pwd)
\end{lstlisting}
...
\begin{lstlisting}[style=sh]
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v2.20260712" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v2.20260712" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v2.20260712" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
Two options select the bus set (in \code{hybrid_traceloops_4_no_mex.m}, or overridden
serially via the global \code{POLEMODE}):

\begin{description}[leftmargin=0em,style=nextline]
\item[\code{'fixed'} (option 1, shipped default)] use a predefined bus set
      \code{param.pole_fixed_buses} (shipped default \code{1:2}, auto-filtered to $\le$ bus
      count).
\item[\code{'random'} (option 2)] draw \code{param.pole_nbus} buses (default 5) at random
      \emph{once per curve} and hold them fixed for that whole trace. Because this build
      seeds the random generator deterministically per \code{(solution, equation)} pair
      (from a solution signature, bus count and equation index), the random set is fully
      reproducible and independent of parallel worker scheduling.
\end{description}
...
directly on line~48 of \code{hybrid_traceloops_4_no_mex.m}.
...
and inside the solution-resolving routine \code{Resolve_no_mex}. Each is a linear system
...
The refactor path is provided by the \code{klurf} MEX, built from SuiteSparse/KLU. This is a
pure-MATLAB build and does \emph{not} ship \code{klurf}, so it runs the dense bordered solve
by default. To enable the acceleration, build \code{klurf} (see the top-level
\code{klurf/README_KLURF.md}) or copy a prebuilt \code{klurf} binary into this folder; the
solver then uses it automatically. Set the global before running to force a mode:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
\code{global USEKLU; USEKLU = 0;} & dense bordered solve (the default here)\\
\code{USEKLU = 1;}  & KLU full factorisation each iteration (needs \code{klu.mexw64})\\
\code{USEKLU = 2;}  & KLU symbolic-once refactor (needs \code{klurf})\\
\code{USEKLU = [];} & auto: refactor if \code{klurf} present, else dense\\
\bottomrule
\end{tabular}
\end{center}
...
\code{hybrid_traceloops_4_no_mex.m} (and mirrored in the parallel drivers). Defaults
...
\textbf{Parameter} & \textbf{Default} & \textbf{Meaning}\\
\midrule\endhead
\code{pole_mode}       & \code{'fixed'}  & \code{'fixed'} = predefined bus set; \code{'random'} = random set drawn once per curve.\\
\code{pole_fixed_buses}& \code{1:2}      & Bus set used when \code{pole_mode='fixed'} (auto-filtered to $\le$ bus count).\\
\code{pole_nbus}       & \code{5}        & Number of buses drawn when \code{pole_mode='random'}.\\
...
\code{hybrid_traceloops_4_no_mex.m} (outer trace-loop orchestrator; builds \code{param},
seeds the RNG, primes the cached operators), \code{holomorphic_hybrid_5_no_mex.m} (one
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v2.20260712}
...
HEBCPF --- MATLAB v2.202607. This document accompanies the solver release and may be
