...
\lhead{\small HEBCPF -- MEX v3.202606}
...
{\large\bfseries Solver line: MEX v3.202606}\\[2pt]
{\large\bfseries Package release: 2026.07.12}\\[4pt]
{\large Windows 64-bit (compiled MEX) --- validated, robustness-hardened build}\\[12pt]
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_MEX_v3.20260712}) anywhere on disk.
  \item Add the folder to the MATLAB path each session:
\begin{lstlisting}
>> cd '<path>\HEBCPF_MEX_v3.20260712'
>> addpath(pwd)
\end{lstlisting}
...
\begin{lstlisting}[style=sh]
:: SERIAL
matlab -sd "<path>\HEBCPF_MEX_v3.20260712" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_MEX_v3.20260712" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_MEX_v3.20260712" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
HEBCPF --- MEX v3.202606. This document accompanies the solver release and may be
