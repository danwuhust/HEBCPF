...
{\large\bfseries Solver line: MATLAB v5}\\[2pt]
{\large\bfseries Package release: 2026.07.15}\\[4pt]
{\large Pure-MATLAB build (no compiled binaries; cross-platform) --- portable v5 reference}\\[12pt]
...
{\small Package release: July 15, 2026}
\end{titlepage}
...
\item[No compiler needed.] This is the pure-MATLAB build: it runs on any MATLAB platform
      ...
      companion \code{HEBCPF_MEX_v5} build; both packages use the same HEBC formulation,
...
\item[Linear-solver boundary.] This implementation uses MATLAB's built-in linear-algebra
      ...
      enable KLU acceleration. Use the companion \code{HEBCPF_MEX_v5} solver when the
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item Unpack this folder (\code{HEBCPF_matlab_v5}) anywhere on disk.
...
:: SERIAL
matlab -sd "<path>\HEBCPF_matlab_v5" -batch "addpath(pwd); run('run_batch.m')"

:: PARALLEL (parfor)
matlab -sd "<path>\HEBCPF_matlab_v5" -batch "addpath(pwd); run('run_batch_par.m')"

:: PARALLEL (parfeval, recommended for big cases)
matlab -sd "<path>\HEBCPF_matlab_v5" -batch "addpath(pwd); run('run_batch_parfeval.m')"
\end{lstlisting}
...
\code{HEBCPF_MEX_v5} solver and its user guide.
...
If you need more speed on Windows x64, the companion release \code{HEBCPF_MEX_v5}
