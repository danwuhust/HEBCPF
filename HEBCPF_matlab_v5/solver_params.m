...
param.maxsolu       = 2000;     % INITIAL cap on solution encounters per continuation loop.
                                %   RATCHETING: a curve exceeding it is left UNCLOSED (Fail=2) and
                                %   the cap doubles for future curves (per worker). Completeness
                                %   preserved -- abandoned-curve members re-reached from other starts.
param.maxsolu_hard  = 5000;     % HARD ceiling on the ratchet (memory bound: each in-flight trace
                                %   holds a (2*bus_n-1) x maxsolu_cur buffer Z on its worker).
